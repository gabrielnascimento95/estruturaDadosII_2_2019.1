...
public class ArvoreB {
	...
	public void insere(Integer k) {
		...
		if (BuscaChave(raiz, k) == null) {
			if (raiz.getN() == 0) {
				...
				NoArvoreB r = raiz;
				if (r.getN() == ordem - 1) {
					NoArvoreB s = new NoArvoreB(ordem);
					raiz = s;
					s.setFolha(false);
					s.setN(0);
					s.getFilho().set(0, r);
	...
	public void divideNo(NoArvoreB pai, int i, NoArvoreB filho) {
	...
	public void insereNoNaoCheio(NoArvoreB x, Integer k) {
		int i = x.getN() - 1;
	...
	public NoArvoreB BuscaChave(NoArvoreB X, Integer k) {
		int i = 1;
	...
	public void Remove(Integer k) {
		...
		if (BuscaChave(this.raiz, k) != null) {
			...
			NoArvoreB N = BuscaChave(this.raiz, k);
			int i = 1;
			...
			if (N.isFolha()) {
				...
				NoArvoreB S = Antecessor(this.raiz, k);
				Integer y = S.getChave().get(S.getN() - 1);
				S.setN(S.getN() - 1);
	...
	private void Balanceia_Folha(NoArvoreB F) {

		if (F.getN() < Math.floor((ordem - 1) / 2)) {
			NoArvoreB P = getPai(raiz, F);
			int j = 1;
	...
	private void Diminui_Altura(NoArvoreB pai) {
		int j;
		NoArvoreB P = new NoArvoreB(ordem);
	...
	private void Balanceia_Esq_Dir(NoArvoreB P, int e, NoArvoreB Esq, NoArvoreB Dir) {
	...
	private void Balanceia_Dir_Esq(NoArvoreB P, int e, NoArvoreB Dir, NoArvoreB Esq) {
	...
	private void Juncao_No(NoArvoreB X, int i) {
		NoArvoreB Y = X.getFilho().get(i - 1);
		NoArvoreB Z = X.getFilho().get(i);
	...
	private NoArvoreB Antecessor(NoArvoreB N, Integer k) {
		int i = 1;
	...
	private NoArvoreB getPai(NoArvoreB T, NoArvoreB N) {
		if (this.raiz == N) {
			return null;
		}
		for (int j = 0; j <= T.getN(); j++) {
			...
			if (!T.getFilho().get(j).isFolha()) {
				NoArvoreB X = getPai(T.getFilho().get(j), N);
				if (X != null) {
					return X;
				}
			}
	...
	public void LimparArvore(NoArvoreB N, int ordem) {
